...
func lexBase64Enc(l *lexer) stateFn {
	if l.pos > l.start {
		if v := l.input[l.start:l.pos]; v != "base64" {
			// fix(yuanjun): add utf8 case
			// 1. replace "utf8" to "charset=utf8"
			// 2. revert to lexInParamAttr
			if strings.ToLower(v) == "utf8" {
				fmt.Println("before replace:", l.input)
				l.input = l.input[:l.start] + strings.Replace(l.input[l.start:], v, "charset=utf8", 1)
				fmt.Println("after replace:", l.input)
...
func lexParamVal(l *lexer) stateFn {
	if l.pos > l.start {
		// fix(yuanjun): if param value is utf8 , set seenUtf8Item flag true
		toLowVal := strings.TrimSpace(strings.ToLower(l.input[l.start:l.pos]))
		if toLowVal == "utf8" || toLowVal == "utf-8" {
			l.seenUtf8Item = true
		}
		l.emit(itemParamVal)
	}
